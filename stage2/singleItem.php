<?php
  ...
  if (isset($_SESSION['username']) && isset($_POST['comments'])) {
    try {
      ...
      $stmtIns = $pdo->prepare('INSERT INTO reviews (Username, Rating, Comments, parkID) VALUES ( :username , 0 , :comments, :id);');
...
<head>
  ...
  <!--TODO <script src="votepage.js"></script> -->
...
<body>
    ...
    <div id="writeComment">
        <form method="post">
            ...
            <p>Leave a Review: </p>
            <textarea name="comments" rows = "3" cols="89"></textarea>
      ...
      <table>
        <?php
          foreach ($reviewResults as $data) {
            echo '<tr><td><h4>'.$data['Username'].' said:</h4><p>'.$data['Comments'].'</p></td></tr>';
          }
        ?>
      </table>
    </div>
