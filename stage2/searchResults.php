...
<body>
      ...
      if(isset($_POST['sort'])){
          try{
              ...
              if ($_POST['sort'] == "ratingL"){
					$sortQ = ' ORDER BY Ratings ASC';
              }

              if($_POST['selectSearch'] == "suburb"){
                $sortQuery = 'SELECT id, Name, Street, Suburb, Latitude, Longitude FROM parks WHERE Suburb LIKE :query'.$sortQuery;
                ...
                $stmt->bindValue(':query','%'.$_POST['suburb'].'%');
                  ...
                  $sortQuery = 'SELECT id, Name, Street, Suburb, Latitude, Longitude FROM parks WHERE Name LIKE :query ' . $sortQuery;
                  $stmt = $pdo->prepare($sortQuery .';');
					...
					if ($_POST['dist'] == "2"){
						...
						foreach($res as $result){
							if (distance($_POST['lat'],$_POST['long'],$result['Latitude'],$result['Longitude']) < 5){
								array_push($final, $result); // Add to array if distance <5km
							}
						}
						$res = $final;
					}
    ...
    <table id="resultsTable">
        <?php
            if(isset($res)){
                foreach($res as $result){
                    echo '<script>addMarker("'.$result['Name'].'",'.$result['Latitude'].','.$result['Longitude'].');</script>';
                    echo '<tr><td><a href="singlePark.php?id'.$result['id'].'">'.$result['Name'].'</a></td></tr>';
                }
            }
        ?>
    </table>
